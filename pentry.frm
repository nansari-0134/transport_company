...
Dim rs1 As New ADODB.Recordset  'listview display records
Dim rs2 As New ADODB.Recordset 'listview item name (add-item frame)
Dim sql As String
Dim sql1 As String 'update qty in item
Dim rs3 As New ADODB.Recordset  'update qty in item
Dim n, a, gen, i, sno, j As Integer
...
' for deleting some rows
If sdata.ListItems(i).Checked = True Then
   Set rs2 = Nothing
   sql = "delete from pdet where pid = " & Text1.Text & " and srno = " & sdata.ListItems(i)
   rs2.Open sql, cn
   Set rs2 = Nothing
   Set rs3 = Nothing
...
If n <> 5 Then
   i = MsgBox("Do You Want To Delete the current record", vbInformation + vbOKCancel, "WARNING")
                    If i = vbOK Then
                      Set rs2 = Nothing
                      sql = "delete from pentry where pid = '" & Text1.Text & "'"
                      rs2.Open sql, cn, adOpenDynamic, adLockOptimistic
                      Set rs2 = Nothing
                      sql = "delete from pdet where pid = '" & Text1.Text & "'"
                      rs2.Open sql, cn, adOpenDynamic, adLockOptimistic
                      Set rs2 = Nothing
                    End If
                    For i = 1 To sdata.ListItems.Count
...
sql = "select * from pdet where pid=" & Text1.Text
...
n = InputBox("Search With :" & vbNewLine & vbNewLine & "1. Purchase Id " & vbNewLine & "2. Purchase Account" _
...
If Text22.Text <> "" Then
 sql = "select * from pentry where paccnt = '" & Text22.Text & "'"
 rsrch.Open sql, cn
 While Not rsrch.EOF
    Set Item = byptlv.ListItems.add(, , rsrch!pid)
...
For i = 1 To byptlv.ListItems.Count
  ...
  Call first_Click
     While Not rs.EOF
       If rs!pid = byptlv.ListItems.Item(i) Then
          sbypt.Visible = False
          byptlv.ListItems.Clear
          Exit Sub
       End If
...
Exit Sub
err:
MsgBox "Please Check Entered Data Again!", vbInformation + vbOKOnly, "Error"
End Sub
...
If n = 1 Then
  'add-save
    'inserting records in pdet
 For i = 1 To sdata.ListItems.Count
 If sdata.ListItems(i).SubItems(2) = "" Then
      sdata.ListItems(i).SubItems(2) = " "
 End If
  sql = "insert into pdet(pid,srno,item,[desc],qty,unit,rate,disc,discamt,amt) " _
        & "values(" & Text1.Text & "," _
        & sdata.ListItems(i) & ",'" _
  ...
  Set rs = Nothing
 Next
  'inserting records in pentry
  sql = "insert into pentry(pid,adate,party_name,paccnt,netamt,add_desc,add_amt,less_desc,less_amt,refno,narratn) " _
        & "values(" & Text1.Text & ",'" _
        & dte.Value & "','" _
        & Text2.Text & "','" _
        & Text3.Text & "'," _
...
ElseIf n = 2 Then
 ...
 End If
 'update pentry
     sql = "update pentry set adate = '" & dte.Value & "',party_name = '" _
           & Text2.Text & "',paccnt = '" _
           & Text3.Text & "',netamt = " _
           ...
           & Text10.Text & "' where pid = " & Text1.Text
      rs.Open sql, cn
      Set rs = Nothing
      Call rec
      rs.MoveLast
